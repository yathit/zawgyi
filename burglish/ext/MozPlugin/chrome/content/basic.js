...
function _id(_s){
	return mmgeeks.doc.getElementById(_s);
}
function isDef(val){
...
function loadjs(_g,_cb,_u1,_u2,_t){
	...
	for( var i =0; i < _g.length ; i++){
	    if(!loaded[_g[i]]){
			var x = mmgeeks.doc.createElement(_t||'script');x.src = _u1 + _g[i] + _u2;x.xid=_g[i];x.id="$"+_g[i]+"$";x.onload=function(){loaded[this.xid]=true;};
			mmgeeks.doc.getElementsByTagName("head")[0].appendChild(x);
		}
	}
