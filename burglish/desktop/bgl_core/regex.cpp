 ...
 * No error checking and no input syntax checking also.
...
void Regex::compile(){
	...
	error=true;
	r.length =0;
	r.mustEnd=false; r.mustStart=false;
...
unsigned int Regex::check(unsigned int chIdx, wchar_t* inputStr, unsigned int strIdx){
	...
	while(repeatCount<r.ch[chIdx].repeatMax){
		...
		for(unsigned int j2=0;j2<r.ch[chIdx].length;j2++){
		...
		if(match && !this->greedy){
			if(r.ch[chIdx].range[0].end== MAXCHARCODE && (r.ch[chIdx].repeatMax-r.ch[chIdx].repeatMin)>0) {
				if(chIdx+1<r.length){
					...
					for(unsigned int j2=0;j2<r.ch[chIdx+1].length;j2++){
...
bool Regex::exec(wchar_t*str){
	...
	do{
		...
		match=true;
		unsigned int regIdx=0; unsigned int grIdx=0; int matchLen=0;
		...
		while(strIdx<strlen && regIdx<r.length){
			...
			* start matching again from next char */
			if(matchLen==0){
				regIdx=0;
				grIdx=0;
				gr.res[resIdx].range[0].start=-1; /* reset group 0 flag */
				if(this->check(regIdx,	str, strIdx)==0) /* only increase if not match  with current char */
					strIdx++;
			}
			...
			while(*rs){
				...
				if(*rs<10){
					...
					if(*rs<=gr.res[resIdx].length){
						...
						for(unsigned int i=gr.res[resIdx].range[(int)*rs].start;i<=gr.res[resIdx].range[(int)*rs].end;i++){
							*ds++=srcStr[i];
						}
